...
var dynamic_data = {
    ...
    'dynamic' : {
        'posts': {
            ...
            'product': {
                'title': [{ key: 'price', label: 'Price' }, {key: 'category', label: 'Main category'}, {key: 'categories', label: 'Categories'}, { key: 'price', label: 'Price' }],
            ...
            'testimonial': {
                'title': [{ key: 'postmeta:mfn-post-author', label: 'Meta Author' }, { key: 'postmeta:mfn-post-company', label: 'Meta Company' }, {key: 'categories', label: 'Categories'}, {key: 'category', label: 'Main category'}],
...
function showGlobals( $section_uid ) {
    ...
    $('.prebuilt-sections-list.global-sections li').each( (index, item) => {
        if($(item).attr('data-id') === $section_uid){
            $(item).addClass('active');
            $(item).find('.text').html('Selected');
        } else {
            $(item).removeClass('active');
            $(item).find('.text').html('Select');
        }
    })
}
...
function init() {
    ...
    if( $content.find('#Header_creative').length ){
        $content.find('#Header_creative').append('<a href="'+mfnvbvars.adminurl+'admin.php?page='+mfnvbvars.be_slug+'-options#header" target="_blank" data-tooltip="Edit Header" class="btn-edit-header" data-position="before">Edit Header</a>');
    } else if( $content.find('#Top_bar').length ){
        $content.find('#Top_bar').append('<a href="'+mfnvbvars.adminurl+'admin.php?page='+mfnvbvars.be_slug+'-options#header" target="_blank" data-tooltip="Edit Header" class="btn-edit-header" data-position="before">Edit Header</a>');
    }
...
let be_navigator = {
    ...
    open: function() {
        ...
        if( $builder.find('.section.vb-item:not(.mfn-global-section)').length ){
            $builder.find('.section.vb-item:not(.mfn-global-section)').each(function() {
                let $section = $(this);
                $('.mfn-navigator .navigator-tree').append( navigatorHtml('Section', $section.attr('data-uid')) );
                if( $section.find('.wrap.vb-item:not(.mfn-global-wrap)').length ){
